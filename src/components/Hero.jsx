...
const Hero = ({ sportMode, setSportMode, navigateTo }) => (
  <header id="home" className="relative min-h-[85vh] flex items-center justify-center overflow-hidden rounded-b-[3rem] md:rounded-b-[5rem] shadow-2xl transition-colors duration-700 bg-slate-900">
    {/* Dynamic Background */}
    ...
    <div className="container mx-auto px-4 relative z-10 text-center text-white pt-20 pb-32">

      {/* ✨ NEW: Interactive Mood Switcher ✨ */}
      <div className="inline-flex bg-white/20 backdrop-blur-lg p-1.5 rounded-full mb-8 border border-white/20 shadow-xl">
        <button 
          onClick={() => setSportMode('ski')}
          className={`px-8 py-3 rounded-full text-base font-bold transition-all duration-300 ${sportMode === 'ski' ? 'bg-emerald-500 text-white shadow-lg transform scale-105' : 'text-white hover:bg-white/10'}`}
        >
          ⛷️ Ski
        </button>
        <button 
          onClick={() => setSportMode('snowboard')}
          className={`px-8 py-3 rounded-full text-base font-bold transition-all duration-300 ${sportMode === 'snowboard' ? 'bg-indigo-500 text-white shadow-lg transform scale-105' : 'text-white hover:bg-white/10'}`}
        >
          🏂 Snowboard
        </button>
      </div>
      ...
      <h1 className="text-5xl md:text-8xl font-black mb-8 leading-tight drop-shadow-xl transition-all duration-500">
      ...
      <p className="text-xl md:text-2xl mb-12 max-w-2xl mx-auto text-gray-100 font-medium leading-relaxed opacity-90">
        {sportMode === 'ski' 
